...
async function callAnthropicOnce(foodText) {
  const response = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({
      ...
      messages: [{ role: 'user', content: `Menu: ${foodText}` }]
    })
  });
  ...
  if (!response.ok) {
    ...
    const err = new Error(`Claude API merespons status ${response.status}${detail ? ' — ' + detail : ''}`);
...
// Memanggil Claude API dengan retry + exponential backoff khusus untuk error 429/529
// (rate limited / server overloaded), yang biasanya sementara dan layak dicoba ulang.
async function callAnthropicWithRetry(foodText, { maxRetries = 3, baseDelayMs = 800 } = {}) {
  let lastErr;
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      return await callAnthropicOnce(foodText);
    } catch (err) {
      lastErr = err;
...
async function estimateMealCalories(foodText) {
  const data = await callAnthropicWithRetry(foodText);
